> [!NOTE]  
> НАП направиха промени в системата. След кратко дебъгване и корекции на скрипта успях да го подкарам с Chrome и сега (февруари 2025 година) би трябвало да работи ([на моя компютър работи](https://www.google.com/search?q=it+works+on+my+computer)). С Firefox попълването на годишна данъчна декларация не работи (нито ръчно, нито със скрипта), НАП съветва да се ползва друг браузър (не се знае дали ще оправят съвместимостта с Firefox). Пускането на скрипта в режим Chrome става от `autopilot_chrome.py` (или `autopilot_slow_chrome.py` в бавен режим - ползвайте този ако има проблем с бързия).
...
Към 5 февруари 2025 г. попълването на годишната данъчна декларация с Firefox не работи. Но може да се ползва Google Chrome. За да стартирате nap-autopilot в режим Google Chrome ползвате стартиращия файл с Chrome в наименованието (`autopilot_chrome.py` или `autopilot_slow_chrome.py`).
