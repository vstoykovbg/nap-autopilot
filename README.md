> [!NOTE]  
> Направих корекции за да работи с формуляра за 2025 година (ГДД за 2025 година, която се подава през 2026 година). Вероятно през 2027 година ще се наложи пак да коригирам скрипта за да работи попълването на декларацията за 2026 година.
...
* Приложение 6 част I - данни за платците на други доходи. Ред 3 - доходи от лихви (код 603) и Ред 6 - доходи от други източници (код 606).
...
* Приложение 9 част II -  Доходи, подлежащи на облагане с данък върху общата годишна данъчна основа, за които се прилага СИДДО.
...
В приложение 9 част II се въвеждат тези доходи от приложение 6 част I, за които се прилага СИДДО и е бил удържан данък при източника.
...
#### Файловете за доходите от лихви и други източници (приложение 6):

```
income_code,income_base
```

Всички числови полета от този файл са суми в левове и трябва да имат не повече от два знака след десетичната точка.

* income_code: Кодът на дохода. Допустими стойности: `603` (доходи от лихви, например от свободен кеш в брокерска сметка или p2p платформа като Mintos и Iuvo) или `606` (доходи от други източници - например bond discount при покупка на облигации в Mintos, капиталова печалба при продажба на облигации в Mintos).
* income_base: Размерът на дохода в левове.

Скриптът разпознава файла по заглавията `income_code` и `income_base` (без да присъства колоната `country`).

#### Файловете за СИДДО на доходите от лихви и други източници (приложение 9):

```
country,income_code,gross_amount_base,tax_base
```

Всички числови полета от този файл са суми в левове и трябва да имат не повече от два знака след десетичната точка.

* country: Държавата, в която е установен платецът на дохода (за IBKR обикновено Ирландия - `IE`, за Mintos - Латвия - `LV`).
* income_code: Кодът на дохода - `603` при доход от лихви или `606` при доход от други източници (вж. по-горе).
* gross_amount_base: Брутният размер на дохода в левове (колона 4 от приложение 9).
* tax_base: Удържаният данък в чужбина в левове (попълва се в колони 8, 9 и 10 от приложение 9).

Скриптът разпознава файла по наличието на колоните `country`, `income_code` и `gross_amount_base` едновременно. За всеки ред фиксира код на дохода `603` в колоната за вид на дохода (колона 5 от Приложение 9).
...
За автоматично конвертиране на датите и попълване на курса на БНБ в таблицата, в която правите обработката на данните, може да ползвате моите скриптове [от тук](https://github.com/vstoykovbg/BNB_currency_rates). Там има и шаблони `.ods` за обработка на дивиденти, смятане на капиталова печалба. Има и скриптове за дивиденти, кешбек и лихви от Trading212, обработка на повечето данни от Interactive Brokers.
...
Има и други начини (платени услуги) за автоматизиране на обработката на данните за данъчната декларация, но не съм ги ползвал и проверявал обстойно как работят:  [NRA Assist](https://nra-assist.com/), [Tax Wizard](https://tax-wizard.eu/en), [Taxable.bg](https://www.taxable.bg/), [Tax123.bg](https://tax123.bg/), [TaxLink.bg](https://taxlink.bg/), [Finoro.bg](https://finoro.bg/), [Tax-invest.com](https://www.tax-invest.com/), [Autotax.bg](https://autotax.bg/), [MinFin](https://www.facebook.com/groups/inv.bg/posts/26585873404339329), [TaxForm Pro](https://taxform.jvmsaas.com/). За криптовалути: [Koinly](https://koinly.io/). За Tax Wizard [имах забележки](https://redtapepayments.blogspot.com/2021/03/blog-post_10.html) последния път като проверявах как смята данъците, дали са го оправили не знам. (NRA Assist мисля, че имаше възможност да генерира csv файлове за nap-autopilot, но не намирам информация на сайта им за това в момента.)

**Давам този списък само за информация, не е препоръка да се ползват, защото не съм запознат детайлно с това как правят изчисленията.** Аз лично не ги ползвам.

Отзиви от потребител ([източник](https://www.facebook.com/groups/clubnainvestitora/posts/26906257235664479?comment_id=26913611934929009)):

> Хора, четири различни инструмента ( такс инвест, такслинк, минфин, finoro ) . Такслинк съвпада с официалния стетмънт като печалба. Финоро и такс инвест показват еднакви резултати но се различават от печалбата в извлечението. Мин фин показва трети резултати. В сички съм заредил извлеченията от всички години. Пълна КАША!

За собствени нужди ползвам [скриптовете и шаблоните, които аз съм направил (отворен код, безплатни са)](https://github.com/vstoykovbg/BNB_currency_rates).
...
Допълнение от 2026 година: в раздела за притежаваните акции и дялове в чужбина (приложение 8) вече има възможност за качване на Excel файл, но не намерих инструкции (в какъв формат трябва да е файла, има ли изобщо изискване за специален формат; не съм тествал функцията за качване на Excel файл).
