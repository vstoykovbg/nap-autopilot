...
Забравих да направя проверка дали датата е въведена коректно, тя се вкарва в друга функция по технически причини (не е с общата функция fill_input). Преглеждайте съобщенията в конзолата, съдържащи "Error" и "date" (пуснете grep накрая на съобщенията за всеки случай). Ако скриптът се провали с попълване на датата ще си проличи по неуспешната валидация от валидатора на НАП. А да се въведе грешна дата е изключително невероятно.
